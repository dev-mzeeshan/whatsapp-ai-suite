...
const BASE_WS_URL = process.env.NEXT_PUBLIC_WS_URL || "ws://127.0.0.1:8000";

// Helper function jo URL ko HTTPS ke mutabiq wss:// me convert karega
const getResolvedWsUrl = (path: string) => {
  let url = `${BASE_WS_URL}${path}`;
...
export function useSidebarSocket() {
  ...
  const connect = useCallback(() => {
    ...
    const fullUrl = getResolvedWsUrl(`/ws/sidebar?token=${token}`);
...
export function useConversationSocket(conversationId: string | null) {
  ...
  const connect = useCallback(() => {
    ...
    const fullUrl = getResolvedWsUrl(`/ws/conversations/${conversationId}?token=${token}`);
    const ws = new WebSocket(fullUrl);
    wsRef.current = ws;
